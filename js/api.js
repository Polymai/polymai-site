(function () {
  ...
  async function callFunction(name, payload, options) {
    ...
    if (!response.ok || body.error) {
      const extras = [];
      if (body.plan) extras.push("plan " + body.plan);
      if (body.expected && body.expected.amount_cents && body.expected.currency) {
        extras.push(String(body.expected.currency).toUpperCase() + " " + (Number(body.expected.amount_cents) / 100) + "/" + (body.expected.interval || "month"));
      }
      if (body.dbField) extras.push(body.dbField);
      const suffix = extras.length ? " (" + extras.join(", ") + ")" : "";
      throw new Error((body.message || body.error || "Request failed.") + suffix);
    }
    return body;
  }
