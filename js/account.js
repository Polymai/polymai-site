(function () {
  ...
  function renderPlanActions(planSlug) {
    ...
    const checkoutAvailable = root.config?.billing?.checkoutAvailable !== false;
    if (current === "studio") {
    ...
    if (current === "pro") {
      ...
      setCheckoutButton("studio", checkoutAvailable ? "Upgrade to Studio" : "Coming soon", !checkoutAvailable, false, true);
      return;
    }
    if (!checkoutAvailable) {
      setCheckoutButton("pro", "Coming soon", true, false, true);
      setCheckoutButton("studio", "Coming soon", true, false, false);
      return;
    }
  ...
  async function startCheckout(plan) {
    if (root.config?.billing?.checkoutAvailable === false) {
      setStatus("Paid plans are coming soon. Free access remains available.", "warning");
      return;
    }
