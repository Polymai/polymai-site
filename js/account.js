(function () {
  ...
  function renderPluginTokens(tokens) {
    ...
    list.innerHTML = rows.map((token) => {
      ...
      const subtitle = [
        token.token_prefix ? `${token.token_prefix}...${token.token_last4 || ""}` : "",
        token.last_used_at ? `last used ${formatDate(token.last_used_at)}` : "not used yet",
      ].filter(Boolean).join(" / ");
      ...
      return `
        <article class="account-row">
          ...
          <div>
            <span class="account-pill">${escapeHtml(status)}</span>
            ${revokeButton}
          </div>
        </article>
      `;
  ...
  function renderSummary(summary) {
    ...
    setText("workspaceName", account.name || "Personal workspace");
