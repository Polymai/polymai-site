(function () {
  ...
  function renderFocusOptions(elements) {
    ...
    const field = elements.focus.closest(".field-select");
    const toolbar = elements.focus.closest(".gallery-toolbar");
    const hasUsefulFilter = focuses.length > 1;
    if (field) field.hidden = !hasUsefulFilter;
    if (toolbar) toolbar.classList.toggle("is-single-field", !hasUsefulFilter);
    if (!hasUsefulFilter) state.focus = "all";
    elements.focus.innerHTML = '<option value="all">All build sources</option>' + focuses.map((focus) => {
  ...
  function renderGallery() {
    ...
    if (elements.status) {
      elements.status.textContent = filtered.length
        ? (visibleItems.length < filtered.length ? (hasActiveFilter ? "Showing the latest matching builds." : "Showing the " + visibleItems.length + " latest builds.") : "Showing matching builds.")
        : "No matching builds.";
    }
    if (elements.count) {
